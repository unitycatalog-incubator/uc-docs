...
import ucSpark1 from '../../../assets/images/UC_naming_conventions_diagram.png';
import ucSpark2 from '../../../assets/images/uc_spark2.png';
import ucSpark3 from '../../../assets/images/uc_spark3.png';
...
<Code code="curl -O https://archive.apache.org/dist/spark/spark-3.5.3/spark-3.5.3-bin-hadoop3.tgz
tar xzf spark-3.5.3-bin-hadoop3.tgz" lang="sh" title="Download Apache Spark 3.5.3 using curl" />
...
<Code code="## S3 Storage Config (Multiple configs can be added by incrementing the index)
...
s3.secretKey.0=&lt;SECRET&gt;" lang="sh" />
</TabItem>

<TabItem label="Azure ADLSgen2">
<Code code="## ADLS Storage Config (Multiple configs can be added by incrementing the index)
adls.storageAccountName.0=&lt;ADLS_STORAGEACCOUNTNAME&gt;
adls.tenantId.0=&lt;ADLS_TENANTID&gt;
adls.clientId.0=&lt;ADLS_CLIENTID&gt;
adls.clientSecret.0=&lt;SECRET&gt;" lang="sh" />
</TabItem>

<TabItem label="Google Cloud Storage">
<Code code="## GCS Storage Config (Multiple configs can be added by incrementing the index)
gcs.bucketPath.0=&lt;GCS_BUCKET&gt;
# Optional (If blank, it will use Default Application chain to find credentials)
gcs.jsonKeyFilePath.0=/path/to/&lt;SECRET&gt;/gcp-key-uc-testing.json" lang="sh" />
</TabItem>
</Tabs>
...
<Code code="cd unitycatalog/
bin/start-uc-server" lang="sh" />
...
<Code code="bin/spark-sql --name 'local-uc-test' --master 'local[*]' --packages 'io.delta:delta-spark_2.13:4.0.0,io.unitycatalog:unitycatalog-spark_2.13:0.3.0' --conf 'spark.sql.extensions=io.delta.sql.DeltaSparkSessionExtension' --conf 'spark.sql.catalog.&lt;catalog_name&gt;=io.unitycatalog.spark.UCSingleCatalog' --conf 'spark.sql.catalog.&lt;catalog_name&gt;.uri=http://localhost:8080' --conf 'spark.sql.catalog.&lt;catalog_name&gt;.token=' --conf 'spark.sql.defaultCatalog=&lt;catalog_name&gt;'" lang="sh" />
</TabItem>

<TabItem label="PySpark">
<Code code="bin/pyspark --name 'local-uc-test' --master 'local[*]' --packages 'io.delta:delta-spark_2.13:4.0.0,io.unitycatalog:unitycatalog-spark_2.13:0.3.0' --conf 'spark.sql.extensions=io.delta.sql.DeltaSparkSessionExtension' --conf 'spark.sql.catalog.&lt;catalog_name&gt;=io.unitycatalog.spark.UCSingleCatalog' --conf 'spark.sql.catalog.&lt;catalog_name&gt;.uri=http://localhost:8080' --conf 'spark.sql.catalog.&lt;catalog_name&gt;.token=' --conf 'spark.sql.defaultCatalog=&lt;catalog_name&gt;'" lang="sh" />
</TabItem>
</Tabs>
...
<Aside type="caution" title="Configuring Spark session catalog">
Depending on your use case, you may need to configure the Spark session catalog, using for example:
<Code code={`--conf "spark.sql.catalog.spark_catalog=org.apache.spark.sql.delta.catalog.DeltaCatalog"`} lang="sh" />
</Aside>
...
<Image src={ucSpark1} alt="Unity Catalog naming conventions diagram" />
...
<Tabs>
<TabItem label="AWS S3">
<Code code="bin/spark-sql --name 's3-uc-test' --master 'local[*]' --packages 'org.apache.hadoop:hadoop-aws:3.4.1,io.delta:delta-spark_2.13:4.0.0,io.unitycatalog:unitycatalog-spark_2.13:0.3.0' --conf 'spark.sql.extensions=io.delta.sql.DeltaSparkSessionExtension' --conf 'spark.hadoop.fs.s3.impl=org.apache.hadoop.fs.s3a.S3AFileSystem' --conf 'spark.sql.catalog.&lt;catalog_name&gt;=io.unitycatalog.spark.UCSingleCatalog' --conf 'spark.sql.catalog.&lt;catalog_name&gt;.uri=http://localhost:8080' --conf 'spark.sql.catalog.&lt;catalog_name&gt;.token=' --conf 'spark.sql.defaultCatalog=&lt;catalog_name&gt;'" lang="sh" />
</TabItem>

<TabItem label="Azure ADLSgen2">
<Code code={`bin/spark-sql --name "azure-uc-test" --master "local[*]" --packages "org.apache.hadoop:hadoop-azure:3.3.6,io.delta:delta-spark_2.13:4.0.0,io.unitycatalog:unitycatalog-spark_2.13:0.3.0" --conf "spark.sql.extensions=io.delta.sql.DeltaSparkSessionExtension" --conf "spark.sql.catalog.<catalog_name>=io.unitycatalog.spark.UCSingleCatalog" --conf "spark.sql.catalog.<catalog_name>.uri=http://localhost:8080" --conf "spark.sql.catalog.<catalog_name>.token=" --conf "spark.sql.defaultCatalog=<catalog_name>"`} lang="sh" />
</TabItem>
...
<Tabs>
<TabItem label="Spark SQL">
<Code code="-- Show schemas (output = default)
SHOW SCHEMAS;

-- Show tables
SHOW TABLES IN default;" lang="sql" />
</TabItem>
...
# Show tables
spark.sql("SHOW TABLES IN default").show()`} lang="python" />
</TabItem>
</Tabs>
...
<Tabs>
<TabItem label="Spark SQL">
<Code code="SELECT * FROM default.marksheet LIMIT 5;" lang="sql" />
</TabItem>

<TabItem label="PySpark">
<Code code={`spark.sql("SELECT * FROM default.marksheet LIMIT 5;").show()`} lang="python" />
</TabItem>
</Tabs>
...
<Tabs>
<TabItem label="Spark SQL">
<Code code="-- Create new schema
CREATE SCHEMA demo;

-- Should now show two schemas: default and demo
SHOW SCHEMAS;" lang="sql" />
</TabItem>
...
# Should now show two schemas: default and demo
spark.sql("SHOW SCHEMAS").show()`} lang="python" />
</TabItem>
</Tabs>
...
<Tabs>
<TabItem label="Spark SQL">
<Code code="-- Create a new table
CREATE TABLE demo.mytable (id INT, desc STRING)
USING delta
LOCATION '&lt;LOCATION&gt;';
-- Example location:
-- LOCATION '/tmp/tables/mytable';" lang="sql" />
</TabItem>
...
\"\"\")
# Example location:
# LOCATION '/tmp/tables/mytable'`} lang="python" />
</TabItem>
</Tabs>
...
-- Read table
SELECT * FROM demo.mytable;`} lang="sql" />
</TabItem>
...
# Read table
spark.sql("SELECT * FROM demo.mytable").show()`} lang="python" />
</TabItem>
</Tabs>
...
<Tabs>
<TabItem label="Spark SQL">
<Code code="-- Update row in table
UPDATE demo.mytable SET id = 5 WHERE id = 4;" lang="sql" />
</TabItem>

<TabItem label="PySpark">
<Code code={`# Update row in table
spark.sql("UPDATE demo.mytable SET id = 5 WHERE id = 4")`} lang="python" />
</TabItem>
</Tabs>
...
<Tabs>
<TabItem label="Spark SQL">
<Code code="-- Delete rows
DELETE FROM demo.mytable WHERE id = 5;" lang="sql" />
</TabItem>

<TabItem label="PySpark">
<Code code={`# Delete rows
spark.sql("DELETE FROM demo.mytable WHERE id = 5")`} lang="python" />
</TabItem>
</Tabs>
...
<Tabs>
<TabItem label="Spark SQL">
<Code code={`-- Create secondary table (we will use this as the source for merge)
...
INSERT INTO demo.srctable VALUES (4, "inserted");`} lang="sql" />
</TabItem>
...
spark.sql("INSERT INTO demo.srctable VALUES (4, 'inserted')")`} lang="python" />
</TabItem>
</Tabs>
...
4       inserted
1       test 1
2       test 2" lang="console" title="Merged Results" />

Drop Table

<Tabs>
<TabItem label="Spark SQL">
<Code code="-- Drop tables
...
<Image src={ucSpark1} alt="UC Spark 1" class="full-width" />
...
<Image src={ucSpark2} alt="UC Spark 2" class="full-width" />
...
<Image src={ucSpark3} alt="UC Spark 3" class="full-width" />
