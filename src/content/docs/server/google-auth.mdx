---
title: Configure Google as the Identity Provider
description: Learn how to configure Google as an external identity provider for Unity Catalog authentication, including setting up OAuth credentials, configuring consent screens, and obtaining client credentials.
---
import { Tabs, TabItem, Aside, FileTree, Code, Steps } from "@astrojs/starlight/components";
import { Image } from 'astro:assets';
import googleOAuthConsentScreen from '../../../assets/images/googleconsole-oauth-consent-screen-sm.png';
import googleCreateOAuthClient from '../../../assets/images/googleconsole-create-oauth-client-id.png';
import googleOAuthClientCreated from '../../../assets/images/googleconsole-oauth-client-created-sm.png';
...
* Go to your project, then select **APIs & Services \> OAuth consent screen**
    ...
    <Image src={googleOAuthConsentScreen} alt="Google Console OAuth Consent Screen" />
...
* Click **APIs & Services \> Credentials** and click the **Create Credentials** button to create an **OAuth client ID**.
    ...
    * **Authorized redirect URIs**: `http://localhost`, `http://localhost:3000` (this is required for the UI)
    ...
    <Image src={googleCreateOAuthClient} alt="Google Console Create OAuth Client ID" />
...
* Once the OAuth client ID is created, a pop-up dialog box will appear containing the **Client ID** and
    **client secret**. Make sure to copy and/or download it (via the Download JSON button), as this will be needed in
    ...
    <Image src={googleOAuthClientCreated} alt="Google Console OAuth Client created" />
