...
description: Learn how to use the Unity Catalog CLI to manage catalogs, schemas, tables, volumes, functions, and ML/AI models through command-line operations.
---
...
<Aside type="note" title="Specify token for authenticated access">
...
<Code code="bin/uc --auth_token $token catalog list" lang="sh" />
...
<Aside type="note" title="Default local Unity Catalog instance">
...
<Code code="bin/uc catalog list \
  [--max_results <max_results>] # (1)" lang="sh" />
...
<Code code="bin/uc catalog create \
  --name <name> \ # (1)
  [--comment <comment>] \ # (2)
  [--properties <properties>] # (3)" lang="sh" />
...
<Code code="bin/uc catalog update \
--name <name> \ # (1)
[--new_name <new_name>] \ # (2)
[--comment <comment>] \ # (3)
[--properties <properties>] # (4)" lang="sh" />
...
<Aside type="note" title="At least one of the optional parameters must be specified.">
</Aside>
...
<Aside title="At least one of the optional parameters must be specified.">
</Aside>

Here's an example:

<Code code={`bin/uc schema update \\
  --full_name my_catalog.my_schema \\
  --new_name my_updated_schema \\
  --comment "Updated Schema" \\
  --properties '{"updated_key": "updated_value"}'`} lang="sh" />

### Delete a Schema

You can delete an existing schema using:

<Code code={`bin/uc schema delete \\
  --full_name <full_name> # (1)`} lang="sh" />
...
<Code code={`bin/uc table list \\
  --catalog <catalog> \\ # (1)
  --schema <schema> \\ # (2)
  [--max_results <max_results>] # (3)`} lang="sh" />
...
<Code code={`bin/uc table create \\
  --full_name <full_name> \\ # (1)
  --columns <columns> \\ # (2)
  --storage_location <storage_location> \\ # (3)
  [--format <format>] \\ # (4)
  [--properties <properties>] # (5)`} lang="sh" />
...
<Code code={`bin/uc table create \\
  ...
  --storage_location "/path/to/storage"`} lang="sh" />

When running against UC server, the storage location can be a local path(absolute path) or an S3 path.
When S3 path is provided, the [server configuration](/server/configuration) will vend temporary credentials to access the S3 bucket and server properties must be set up accordingly.

### Read a Delta Table

You can read a Delta table using:

<Code code={`bin/uc table read \\
  --full_name <full_name> \\ # (1)
  [--max_results <max_results>] # (2)`} lang="sh" />
...
<Code code={`bin/uc table write \\
  --full_name <catalog>.<schema>.<table> # (1)`} lang="sh" />
...
<Code code={`bin/uc volume create \\
  --full_name <full_name> \\ # (1)
  --storage_location <storage_location> \\ # (2)
  [--comment <comment>] # (3)`} lang="sh" />
...
2. `storage_location`: The storage location associated with the volume. When running against UC OSS server,
   ...
   [server configuration](/server/configuration) will vend temporary credentials to access the S3 bucket and server properties must be set up accordingly. When running against Databricks Unity Catalog, the storage location for EXTERNAL volume can only be an S3 location which has been configured as an `external location` in your Databricks workspace.
...
<Code code={`bin/uc volume update \\
...
<Code code={`bin/uc volume update \\
  ...
  --comment "Updated Volume"`} lang="sh" />

### Read Volume content

Here's how you can access the contens of a volume:

<Code code={`bin/uc volume read \\
  --full_name <catalog>.<schema>.<volume> \\ # (1)
  [--path <path>] # (2)`} lang="sh" />
...
<Code code={`bin/uc volume delete \\
  --full_name <full_name> # (1)`} lang="sh" />
...
<Code code={`bin/uc function create \\
  --full_name <full_name> \\ # (1)
  --input_params <input_params> \\ # (2)
  --data_type <data_type> \\ # (3)
  --def <definition> \\ # (4)
  [--comment <comment>] \\ # (5)
  [--language <language>] # (6)`} lang="sh" />
...
<Code code={`bin/uc function call \\
  --full_name my_catalog.my_schema.my_function \\
  --input_params "1,2"`} lang="sh" />

### Delete a Function

You can delete a function using:

<Code code={`bin/uc function delete \\
  --full_name <full_name> # (1)`} lang="sh" />
...
<Code code={`bin/uc registered_model list \\
  --catalog <catalog_name> \\ # (1)
  --schema <schema_name> # (2)`} lang="sh" />
...
<Code code={`bin/uc model_version list \\
  --full_name <full_name> # (1)`} lang="sh" />
...
<Code code={`bin/uc registered_model get \\
  --full_name <full_name> # (1)`} lang="sh" />
...
<Code code={`bin/uc registered_model update \\
  --full_name <full_name> \\ # (1)
  --new_name <new_name> \\ # (2)
  [--comment <comment>] # (3)`} lang="sh" />
...
Read more in the [Models](/usage/models) documentatino.
...
<Aside type="note" title="Different look for users CLI commands">
...
<Code code="bin/uc user create [options]" lang="sh" />

_Required Params:_

    <Code code={`--name: The name of the entity.
--email: The email address for the user`} lang="sh" />

_Optional Params:_

    <Code code={`--server: UC Server to connect to. Default is reference server.
--auth_token: PAT token to authorize uc requests.
--external_id: The identity provider's id for the user.
--output: To indicate CLI output format preference. Supported values are json and jsonPretty.`} lang="text" />

### Delete User

<Code code="bin/uc user delete [options]" lang="sh" />

_Required Params:_

    <Code code="--id: The unique id of the user" lang="text"  />
...
<Code code="bin/uc user get [options]" lang="sh" />

_Required Params:_

    <Code code="--id: The unique id of the user" lang="sh" />

_Optional Params:_

    <Code code={`--server: UC Server to connect to. Default is reference server.
--auth_token: PAT token to authorize uc requests.
--output: To indicate CLI output format preference. Supported values are json and jsonPretty.`} lang="sh" />

### List Users

<Code code="bin/uc user list [options]" lang="sh" />

_Required Params:_

    <Code code="None" lang="sh" />
...
--count: Desired number of results per page`} lang="text" />

### Update User

<Code code="bin/uc user update [options]" lang="sh" />
...
--external_id: The identity provider's id for the user
--email: The email address for the user`} lang="text" />
...
<Code code="bin/uc metastore get" lang="sh" />
