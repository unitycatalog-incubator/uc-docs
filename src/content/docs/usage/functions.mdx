...
description: Learn how to store, access and govern functions in Unity Catalog. Functions allow you to save reusable logic and apply permissions and filters.
---

import { Tabs, TabItem, Aside, FileTree, Code } from "@astrojs/starlight/components";
import ucFunctions from '../../../assets/images/uc_functions.png';
...
<Image src={ucFunctions} alt="UC Functions" />
...
<Aside type="note">
If this is your first time spinning up a UC server, you might want to check out the [Quickstart](/getting-started/quickstart/) first.
</Aside>
...
<Code code="bin/start-uc-server" lang="sh" />
...
<Code code="bin/uc function list --catalog unity --schema default" lang="sh" />
...
<Code code="bin/uc function get --full_name unity.default.sum" lang="sh" />
...
<Code code="bin/uc function call --full_name unity.default.sum --input_params '1,2,3'" lang="sh" />
...
<Code code={`def myPythonFunction(n_sides, n_dice):
    import numpy as np
    rolls=np.random.randint(1, n_sides + 1, size=n_dice)
    return print(rolls)"`} lang="python" />
